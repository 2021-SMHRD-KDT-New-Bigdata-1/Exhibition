						...
						<div class="inner">
                           ...
                           <% }}%>
							...
							if(like_origin.contains(vo.getMB_nick())){%>
								<div id="like_btn"><button onclick='location.href="adlikePostSaveLink?seq=<%=seq%>&nick=<%=vo.getMB_nick()%>"'>좋아요 취소</button><%=list.get(0).getLike_cnt() %></div>
							<%}else{%>
								<div id="like_btn"><button onclick='location.href="adlikePostSaveLink?seq=<%=seq%>&nick=<%=vo.getMB_nick()%>"'>좋아요</button><%=list.get(0).getLike_cnt() %></div>
							<%} %>
